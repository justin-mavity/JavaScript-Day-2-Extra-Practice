...
vacations.forEach( function ( item ) {
    if ( item.beach === true && item.temperature > 90 ) {
...
vacations.forEach( function ( item ) {
    if ( item.beach === true && item.wifi === "strong" && item.hiking === true ) {
        developerOptions.push( item.city );
    }
} )
...
function average( arr ) {
     const ave = vacations.reduce( function ( acc, item ) {
        return acc + item.overall_rating;
     }, 0 );
    return ave;
}
console.log( average( vacations ) );
